...
import FullscreenHotbar from './FullscreenHotbar';
import { Terminal, Monitor, Minimize2, Maximize2, RefreshCw } from 'lucide-react';
...
export default function DetachedDeskShell({ deskId, deskName }: DetachedDeskShellProps) {
  ...
  return (
    ...
    >
      {/* If Fullscreen: Render Fullscreen Hotbar; otherwise standard Detached Top Bar */}
      {isFullscreen ? (
        <FullscreenHotbar />
      ) : (
        <div className="h-7 bg-[#0b0d13] border-b border-border/80 flex items-center justify-between px-3 shrink-0 select-none text-xs z-30">
          ...
          {/* Right: Fullscreen Toggle, Restart to Update, Dock Back & Close Window */}
          <div className="flex items-center space-x-2 text-[10px]">
            ...
            <button
              ...
              className="flex items-center space-x-1 px-2 py-0.5 rounded bg-surface hover:bg-surface-hover text-text border border-border/60 transition-colors cursor-pointer"
